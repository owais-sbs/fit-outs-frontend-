...
export default function TaskChatPanel({
  ...
  const send = async () => {
    ...
    setError("");
    try {
      await postTaskMessage(projectId, taskId, body.trim(), file, null);
      setBody("");
      setFile(null);
  ...
  return (
    <div className="flex min-h-[480px] flex-1 flex-col overflow-hidden rounded-xl border border-border/60 bg-[#e5ddd5]/40 dark:bg-muted/30">
      ...
      <div className="flex-1 space-y-1.5 overflow-y-auto px-3 py-3">
        {messages.length === 0 ? (
          ...
          messages.map((m) => {
            ...
            const hasFile = Boolean(m.referencedVersionId || m.attachmentName);
            return (
                ...
                >
                  ...
                  {hasFile && (
                    href ? (
                      ...
                      <p className={`mt-1.5 flex items-center gap-1.5 text-xs ${mine ? "opacity-90" : "text-muted-foreground"}`}>
                        <FileText className="h-3.5 w-3.5 shrink-0" />
                        <span className="truncate">{fileLabel(m)}</span>
                      </p>
                    )
